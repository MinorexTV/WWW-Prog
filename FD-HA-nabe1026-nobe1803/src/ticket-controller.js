...
export function tagesticket(ctx) {
    ...
    ctx.response.body = ctx.nunjucks.render("tagesticket.html", {authenticated: auth});
  ...
  export function vipticket(ctx) {
    debug("@index. ctx %O", ctx.request.url);
    const auth = ctx.session.userId;
    ctx.response.body = ctx.nunjucks.render("vipticket.html", {authenticated: auth});
    ctx.response.status = 200;
    ctx.response.headers["content-type"] = "text/html";
    return ctx;
  }
  ...
  export async function tagesticketSubmit(ctx) {
    ...
    const order = {name: name, surname: surname, email: email, foodpass: foodpass, tickettype: tickettype};
  ...
  export function confirmation(ctx) {
    debug("@index. ctx %O", ctx.request.url);
    const auth = ctx.session.userId;
    ctx.response.body = ctx.nunjucks.render("confirmation.html", {authenticated: auth});
    ctx.response.status = 200;
    ctx.response.headers["content-type"] = "text/html";
    return ctx;
  }
